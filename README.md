...
テーマ本体を変更せず、Typoraのテーマフォルダに `japanese-print.user.css` または `japanese-academic.user.css` を作ると、更新時にも設定を保てます。`base.user.css` はこの2テーマには適用されないため、変更するテーマと同じ名前を使用してください。
...
印刷時の最初のH1は文書表題として扱い、13.5pt・折返しなしで表示します。CSSだけでは文字数や実際の描画幅を測って自動縮小できないため、非常に長い表題では `--jp-print-title-font-size` を小さくしてください。編集画面では、折り返す場合も行長が偏りにくいよう `text-wrap: balance` を使用します。
